...
query_enc="$1"
echo query0:$query_enc >> /tmp/debug.txt
...
captive-portal-custom=captive-portal-custom
...
passwordmd5="$(echo $query | awk -F ', ' '{print $6;}' | awk -F 'passwordmd5=' '{print $2;}' | md5sum | awk '{print $1}')"
phone="+$(echo $query | awk -F ', ' '{print $7;}' | awk -F 'phone= ' '{print $2;}')"

# Define some common html as the first part of the page to be served by NDS
#
# Note this example uses the default splash.css provided by NDS and uses splash.png
...
header="
	...
	<img src=\"/images/splash.png\" alt=\"Splash Page: For access to the Internet, please click Continue.\">
	<hr>
"
...
footer="
	<img style=\"height:60px; width:60px; float:left;\" src=\"/images/splash.png\" alt=\"Splash Page: For access to the Internet.\">
...
login_form="
	...
	<br>Name:<br>
	<input type=\"text\" name=\"username\" value=\"$username\" autocomplete=\"on\" ><br>
	<br>E-mail:<br>
	<input type=\"email\" name=\"emailaddr\" value=\"$emailaddr\" autocomplete=\"on\" ><br>
	<br>Password:<br>
	<input type=\"password\" name=\"passwordmd5\" value=\"****\" autocomplete=\"on\" ><br>
        <br>Phone:<br>
        <input type=\"phone\" name=\"phone\" value=\"$phone\" pattern=\"+[0-9]{1,3}[0-9]{5,99}\" required autocomplete=\"on\" placeholder=\"$phone\" ><br>
...
#
okauth=$(grep  "^$username;$emailaddr;$phone;$passwordmd5$" /root/auth.csv  -c | head -c1)

if [ -z $username ] || [ -z $emailaddr ] || [ -z $passwordmd5 ] || [ -z $phone ]; then

	if [ "$okauth" -eq "0" ]; then
		echo "<big-red>Welcome!</big-red><italic-black><p> To access the Internet you need to enter the right data </italic-black><hr>"
		echo -e $login_form
	else
		echo "<big-red>Welcome!</big-red><italic-black><p> To access the Internet you must enter your name and email address.</italic-black><hr>"
		echo -e $login_form
	fi
else
	if [ "$okauth" -eq "0" ]; then
                echo "<big-red>Welcome!</big-red><italic-black> To access the Internet you need to enter data.</italic-black><hr>"
                echo -e $login_form
	else
		...
		echo "<br><italic-black> Your News or Advertisement could be here, contact the owners of this Hotspot to find out how!</italic-black>"
